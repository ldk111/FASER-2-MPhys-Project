...
inputdir = "nmuon_acts_final.root"
outdir ="nmuon_filter_final"
nmuon = True
ylim = 2000
zlim = 1000

def GenerateACTSData(inputdir, outdir, ylim, zlim, nmuon = True):
    ...
    output = r.TFile(outdir + ".root", "RECREATE")
    ...
    vy_mu = ak.flatten(tree_mu["vy"].array())
    vz_mu = ak.flatten(tree_mu["vz"].array())

    mask = (np.abs(vz_mu) < zlim) & (np.abs(vy_mu) < ylim)

    vy_mu = vy_mu[mask]
    vz_mu = vz_mu[mask]

    vx_mu = ak.flatten(tree_mu["vx"].array())[mask]
    px_mu = ak.flatten(tree_mu["px"].array())[mask]
    py_mu = ak.flatten(tree_mu["py"].array())[mask]
    pz_mu = ak.flatten(tree_mu["pz"].array())[mask]
    p_mu = ak.flatten(tree_mu["p"].array())[mask]
    eta_mu = ak.flatten(tree_mu["eta"].array())[mask]
    theta_mu = ak.flatten(tree_mu["theta"].array())[mask]
    phi_mu = ak.flatten(tree_mu["phi"].array())[mask]
    pt_mu = ak.flatten(tree_mu["pt"].array())[mask]
...
GenerateACTSData(inputdir, outdir, ylim, zlim, nmuon)