...
def Generate_DataFrame_From_ROOT(input_dir, i):
    ...
    df_columns = [
                "QOP_FIT", "PHI_FIT", "THETA_FIT", "ETA_FIT", "PT_FIT", "P_FIT", "PZ_FIT", "Q_FIT",
    ...
    df_data = [
                QOP_FIT, PHI_FIT, THETA_FIT, FIT_ETA[:,5], FIT_PT[:,5], P_FIT, PZ_FIT, Q_FIT,
                P_TRUTH, Q_TRUTH, PX_TRUTH, PY_TRUTH, PZ_TRUTH, PHI_TRUTH, THETA_TRUTH, 
                CHI2SUM, NDF, 
    ...
    df.dropna(subset=["GLOBAL_Z_HIT_6"], inplace=True)
...
def GenerateACTSData(df, outdir, nmuon = True):
    ...
    if tree:
        ...
        tree.SetBranchAddress("vertex_primary", vertex_primary)
        tree.SetBranchAddress("vertex_secondary", vertex_secondary)
        tree.SetBranchAddress("particle", particle)
        tree.SetBranchAddress("generation", generation)
        tree.SetBranchAddress("sub_particle", sub_particle)
    ...
    vx_mu = df["GLOBAL_X_HIT_1"].values-0.0005
    vy_mu = df["GLOBAL_Y_HIT_1"].values-0.0005*df["FIT_PY_1"].values/df["FIT_PX_1"].values
    vz_mu = df["GLOBAL_Z_HIT_1"].values-0.0005*df["FIT_PZ_1"].values/df["FIT_PX_1"].values
    px_mu = df["FIT_PX_1"].values
    py_mu = df["FIT_PY_1"].values
    pz_mu = df["PZ_FIT"].values
    p_mu = df["P_FIT"].values
    eta_mu = df["ETA_FIT"].values
    theta_mu = df["THETA_FIT"].values
    phi_mu = df["PHI_FIT"].values
    pt_mu = df["PT_FIT"].values
    q_value = df["Q_FIT"].values
    particle_type_value = -13*df["Q_FIT"].values
    ...
    for i in range(n_list_muon):
        ...
        particle_type.push_back(particle_type_value[i])  # Assign 13 or -13 depending on the charge
        process.push_back(0)
        ...
        q.push_back(q_value[i]) # charge of muon
